...
- A notification backend is configured on the server and delivers alerts (see [Configure alerts](alerts.md)).
...
Only needed for a board built with the K-wire interface, if you want engine data and fault codes (see [Deployment configuration](configuration.md)). Firmware 0.4.x reads nothing over CAN.
...
The first two are covered in [Mounting recommendations](mounting.md).
