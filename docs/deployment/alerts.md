...
To change the accelerometer alert priorities for a vehicle, set these in its `remote.conf` section and publish with `push_fw.sh` (see [Deployment configuration](configuration.md)):
...
3. Test a parked movement alert as described in [Test drive](test-drive.md).
