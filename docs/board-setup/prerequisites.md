...
- **A current-limited 12V bench supply.** The 50mA limit is only for the first power-up checks. Before running firmware, raise it to around 300mA: a modem registering on the network and transmitting draws more than 50mA from the 12V input, and a supply sitting in current limit makes the board brown out and reset.
