...
Complete [Board setup prerequisites](prerequisites.md) first. Commands on this page run in `firmware/`.
...
| Symptom | Check |
|---|---|
| `pyocd list` finds no probe | The cable carries data. On Linux, the udev rule from the [prerequisites](prerequisites.md#linux-access-to-the-connect-kit) is in place. In a virtual machine, the Connect Kit is connected to it. |
