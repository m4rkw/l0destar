...
This page installs it with Docker. One image, `m4rkw/l0destar`, holds the server and the MariaDB database it uses, and everything the server keeps is stored in a directory on the host. [Server configuration](configuration.md) explains the settings and [Server deployment](deployment.md) covers HTTPS, backups and upgrades. Read [Server security](security.md) before exposing anything to a network.
...
- A public **IPv4** address the trackers can reach on UDP 65480 and TCP 65481, directly or through port forwarding, and a DNS name with an A record pointing at it. The firmware cannot use IPv6; see [Telemetry port](telemetry-port.md).
- An HTTPS address for the web interface, because passkeys only work in a secure context: Tailscale ([Server security](security.md), recommended) or nginx with a public certificate ([Server deployment](deployment.md)).
...
`gentoken.py` prints a bearer token for the HTTP API. `tools/command.py` uses the first token in the database to queue commands, so create one even if nothing else will use the API, and treat it like a password ([Server security](security.md)).
...
Then start it with the same `docker run` command as above: Docker uses the local image instead of pulling one. To upgrade it later, pull the repository with `git -C l0destar pull`, build again and recreate the container as in [upgrading](deployment.md#upgrading), leaving out `docker pull`.
