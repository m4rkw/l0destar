...
- **The certificate name.** The update client checks the TLS certificate against your CA and the hostname, so the certificate must be issued for `CONFIG_APP_SERVER_HOST` - the `L0DESTAR_HOSTNAME` the server was first started with ([Server installation](/server/installation.md)).
...
The tracker's own console shows what it resolved and sent; see [Verifying telemetry](/board-setup/verifying-telemetry.md).
