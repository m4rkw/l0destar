...
are on [Device settings and commands](/reference/device-settings.md).
...
on [Board setup: Prerequisites](/board-setup/prerequisites.md). That logic is in `ncs_env.sh`,
...
same decision see [Hardware](/reference/hardware.md#interface-selection-pads).
...
[alert catalogue](/deployment/alerts.md#alert-catalogue).
...
created on its first start (see [Server: Installation](/server/installation.md#give-the-build-machine-the-ca)). When the tracker firmware boots it writes the
